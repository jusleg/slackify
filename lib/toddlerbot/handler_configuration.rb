...
module Toddlerbot
  class HandlerConfiguration
    ...
    def call_command(message, params)
      ...
      if command.nil?
        UnhandledHandler.unhandled(params)
      else
    ...
    def environment_configurations
      @bot_id =
        ...
        when 'production', 'staging', 'development'
          Toddlerbot.configuration.slack_client.auth_test['user_id']
        when 'test'
          ''
        end
    end
    ...
    def generate_handler_from_yaml(handler_yaml)
      Toddlerbot::HandlerValidator.verify_handler_integrity(handler_yaml)
